  ...
  <!-- 
	...
	<!-- style="background-image: url(images/blog-2.jpg);" -->
		<div class="container-fluid">
			...
			<div class="row">
				<?php
	$stmt=$admin->ret("SELECT * from doctors inner join timing on doctors.d_id=timing.did");
		...
		echo '
				...
				<div class="col-md-3 animate-box text-center">
					<div class="doctor">';?>
						...
						<?php echo '
						<h3><a href="#"></a>'.$row['d_name'].'</h3>
						<span><i>'.$row['d_qualfn'].'</i></span>
						<i>Specialist :  '.$row['d_splist'].'</i><br><br>
						<label>Contact :  '.$row['d_phno'].'</label>
						<label>Mail :  '.$row['d_mail'].'</label><br><br>
						<label>Clinic | Hospital :  '.$row['d_clnc'].'</label>
						<label>Clinic location :  '.$row['d_clncloc'].'</label>
						';?>
							<?php
						
		echo '
						<span>'.$row['from1'].' to '.$row['to1'].'</span>
						';?>
						
					</div>
					<a href="appointment.php?id=<?php echo $row['did'];?>" style="margin-top:-80px;" class="btn btn-warning">Take Appointment</a>
				</div> 
