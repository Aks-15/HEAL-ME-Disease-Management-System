  ...
  <!-- 
	...
	$row=$stmt->fetch(PDO::FETCH_ASSOC); 
		echo '
				...
				<div class="col-md-3 animate-box text-center" style="margin-left:500px;margin-top:-80px;">
					<div class="doctor">';?>
					<form action="../controller/editpprofile.php" method="POST">
						<div class="staff-img">
							<img src="../images/person3.jpg" class="staff-img">
						</div>
						
						<input type="hidden" name="cid" value="<?php echo $cid;?>">
						<?php echo '

						<input type="text" autocomplete="off" pattern="^[A-Za-z\s]+$" title="Enter only characters" name="cname" value="'.$row['name'].'">
						<input type="text" name="gender" value="'.$row['gender'].'">
						<input type="text" autocomplete="off" pattern="^[A-Za-z.,-]+$" name="adds" value="'.$row['adds'].'">
						<input type="text" autocomplete="off" pattern="[0-9]{10}" title="Enter only 10 digit phone number" name="ph_no" value="'.$row['ph_no'].'">
						<input type="text" autocomplete="off" pattern="[a-z0-9._%+-]+@[a-z.-]+\.[a-z]{2,4}$" title="xyz@something.com" name="email" value="'.$row['email'].'">
